...
#Reference functions
#catogorical_col=[item[0] for item in final_data.dtypes if item[1].startswith('string')]
#spark.conf.set("spark.sql.autoBroadcastJoinThreshold", -1)
#model_training_data.select([F.count(F.when(F.isnan(c), c)).alias(c) for c in model_training_data.columns])
...
def apply_Regent_filters(df_ind_mag_info):
  Active_Subscriber=df_ind_mag_info.filter((F.col('country_code')=='U')&(F.col('type_of_address')=='C')&
                            (((F.col('ib_household_income')>5)&((F.col('age')>50)|(F.col('age')=='0')))|
...
Active_Subscriber=apply_Regent_filters(df_ind_mag_info)
